...
var handleAuthentication = function(req, options) {
  ...
  var client = ldapModule.createClient({
    url: this.options.url
  });

  // TODO: genericize
  var opts = {
    filter: '(&(samaccountname=' + username + ')(objectclass=organizationalPerson))',
    scope: 'sub'
  };

  client.bind('CORP\\' + username, password, function (err, res) {
    if(err) {
      this.fail({message: options.invalidCredentials || 'Invalid username/password'}, 401);
      return;
    }

    var that = this;
    client.search('DC=corp,DC=pvt', opts, function (err, search) {
      search.on('searchEntry', function (entry) {
        var user = entry.object;

        if (!user) {
          that.fail({message: options.userNotFound || 'Invalid username/password'}, 401);
            return;
        }
